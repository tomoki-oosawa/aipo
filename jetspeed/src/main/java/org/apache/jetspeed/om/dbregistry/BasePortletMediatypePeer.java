...
public abstract class BasePortletMediatypePeer extends BasePeer {
   ...
   *
   */
  private static final long serialVersionUID = -2396152796747132952L;
   ...
   *           into a TorqueException.
   */
  public static List<BaseObject> doSelect(PortletMediatype obj)
      throws TorqueException {
    ...
    for (int i = 0; i < rows.size(); i++) {
      ...
      for (int j = 0; j < results.size(); j++) {
        ...
        PortletDbEntry temp_obj2 =
          ((BasePortletMediatype) temp_obj1).getPortletDbEntry();
   ...
   *           into a TorqueException.
   */
  protected static List<BaseObject> doSelectJoinMediatype(Criteria c)
      throws TorqueException {
    // Set the correct dbName if it has not been overridden
    // c.getDbName will return the same object if not set to
    // another value so == check is okay and faster
    ...
    for (int i = 0; i < rows.size(); i++) {
      ...
      for (int j = 0; j < results.size(); j++) {
        BaseObject temp_obj1 = results.get(j);
        Mediatype temp_obj2 = ((BasePortletMediatype) temp_obj1).getMediatype();
        if (temp_obj2.getPrimaryKey().equals(obj2.getPrimaryKey())) {
          newObject = false;
          temp_obj2.addPortletMediatype(obj1);
          break;
