 ...
 * @param portlet_id
 */
aipo.blog.finFilterSearch = function(portlet_id) {
	if (aipo.userAgent.isAndroid4_0()) {
		var q = dojo.byId("q" + portlet_id);
		var filters = dojo.byId('filters_' + portlet_id);
		if (filters && q) {
...
aipo.blog.filterSetDefault = function(portlet_id, type) {
	var ul = dojo.query("ul.filtertype[data-type=" + type + "]", dojo
			.byId("searchForm_" + portlet_id))[0];
...
aipo.blog.onChangeFilter = aipo.blog.onChangeSearch = function(baseuri,
		portlet_id) {
...
aipo.blog.expandImageWidth = function(img) {
	...
	if (!class_name.match(/width_auto/i)) {
		img.className = img.className
				.replace(/\bwidth_thumbs\b/g, "width_auto");
	} else {
		img.className = img.className
				.replace(/\bwidth_auto\b/g, "width_thumbs");
	}
}
...
aipo.blog.ExpandImage = function(url) {
	...
	var popup = window.open("image", "_blank", "left=+x+", "top=+y+",
			"width=+imwidth+", "height=+imheight+", "scrollbars=yes",
			"resizable=yes");
	popup.window.document.open();
	popup.window.document
			.write('<html><head><title>'
					+ im.alt
					+ '</title></head><body style="margin:0;padding:0;border:0;"><img src="'
					+ im.src + '" width="100%" alt="" /></body></html>');
	popup.window.document.close();
}

aipo.blog.formSwitchThemaInput = function(button) {
	if (button.form.is_new_thema.value == 'TRUE'
			|| button.form.is_new_thema.value == 'true') {
...
aipo.blog.delCommentReply = function(button, id, indicator_id, p_id) {
	...
	if (confirm(val1)) {
		...
		var url = button.form.action + '&mode=commentdel&' + button.name
				+ '=1&comment_id=' + id;
		aimluck.io.disableForm(button.form, true);
		aimluck.io.setHiddenValue(button);
		button.form.action = url;
		aimluck.io.submit(button.form, indicator_id, p_id,
