...
aipo.timeline.addHiddenValue = function(form, name, value) {
  ...
  if (name == "mode" && value == "comment" || value == "like"
      && dojo.byId("tutorial_timeline2") != null) {
    ...
    dojo.xhrPost({
      ...
      load : function(data, event) {
        if (data != "error") {
          ...
          // aipo.timeline.refreshImageList(pid, dojo.byId("TimelinePage_" + pid
...
aipo.timeline.onKeyUp = function(pid, tid, e) {
  ...
  var shadowVal = val.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/&/g,
      '&amp;').replace(/\n$/, '<br/>&nbsp;').replace(/\n/g, '<br/>').replace(
      / {2,}/g, function(space) {
  ...
  if (mode == 'like') {
    ...
    if (typeof onsubmit == "string") {
      ...
      var onsubmitString = onsubmit.toString().replace("\'like\'",
          "\'dislike\'");
      onsubmitString = onsubmitString.substring(
          onsubmitString.indexOf("{") + 1, onsubmitString.lastIndexOf("}") - 1);
...
aipo.timeline.addText = function(form, pid) {
  if (dojo.byId("tlInputClip_" + pid).innerHTML.length > 1) {
    ...
    if (dojo.byId("tlClipImage_" + pid + "_" + page.value) != null
        && dojo.byId("tlClipImage_" + pid + "_" + page.value).style.display != "none") {
      aipo.timeline.addHiddenValue(form, "tlClipImage", dojo
          .byId("tlClipImage_" + pid + "_" + page.value).children[0].name);
    }
    aipo.timeline.addHiddenValue(form, "tlClipTitle", dojo.byId("tlClipTitle_"
        + pid).children[0].innerHTML);
    if (dojo.byId("tlClipUrl_" + pid).children[0].innerHTML) {
      aipo.timeline.addHiddenValue(form, "tlClipUrl", dojo.byId("tlClipUrl_"
          + pid).children[0].getAttribute("href"));
    }
    aipo.timeline.addHiddenValue(form, "tlClipBody", dojo.byId("tlClipBody_"
        + pid).innerHTML);
  }
}
...
aipo.timeline.write = function(inthis, indicator_id, pid) {
  ...
  if (dojo.byId("tutorial_timeline1") != null) {
    dojo.byId("tutorial_timeline1").style.display = "none";
  }
}
...
aipo.timeline.setMinHeight = function(pid) {
  var min = 0;
  if (document.all) {
    min += (document.documentElement.clientHeight - dojo.byId("message_" + pid)
