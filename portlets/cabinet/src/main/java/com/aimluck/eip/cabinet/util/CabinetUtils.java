...
public class CabinetUtils {
   ...
   * @return
   */
  public static EipTCabinetFolder getEipTCabinetTargetFolder(RunData rundata,
      Context context) {
    String folderid =
      ALEipUtils.getTemp(rundata, context, KEY_TARGET_FOLDER_ID);
  ...
  /**
   * カレントフォルダオブジェクトモデルを取得します。 <BR>
   * 
   * @param rundata
   * @param context
   * @return
   */
  public static EipTCabinetFolder getEipTCabinetCurrentFolder(RunData rundata,
      Context context) {
    String folderid =
      ALEipUtils.getTemp(rundata, context, KEY_CURRENT_FOLDER_ID);
