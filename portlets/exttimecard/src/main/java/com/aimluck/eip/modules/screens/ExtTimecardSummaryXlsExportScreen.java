...
public class ExtTimecardSummaryXlsExportScreen extends ALXlsScreen {
  ...
  public void init(RunData rundata, Context context)
    ...
    target_group_name = rundata.getParameters().getString(
  ...
  private void setupTimecardSheet(RunData rundata, Context context,
    ...
    String[] headers = {
        "氏名",       "年",         "月",       "勤務形態",     "出勤日数",
        "就業時間",   "残業日数",   "残業時間", "休出日数",     "休出時間",
        "遅刻日数",   "早退日数",   "欠勤日数", "有休日数",     "代休日数",
        "その他日数", "未入力"
        };
    // 0：日本語，1：英数字
    short[] cell_enc_types = {
        HSSFCell.ENCODING_UTF_16, HSSFCell.CELL_TYPE_NUMERIC, HSSFCell.CELL_TYPE_NUMERIC, HSSFCell.ENCODING_UTF_16, HSSFCell.CELL_TYPE_NUMERIC,
    ...
    List daykeys = listData.getGroupExtTimecards();
    int daykeysize = daykeys.size();
    for (int i = 0; i < daykeysize; i++) {
      tclistrd = (ExtTimecardSummaryResultData) daykeys.get(i);
      ...
      String early_leaving_day = tclistrd.getEarlyLeavingDay().getValueAsString();//早退日数
      ...
      String compensatory_holiday = tclistrd.getCompensatoryHoliday().getValueAsString();//代休日数
      String other_day = tclistrd.getOtherDay().getValueAsString();//その他日数
      String noinput = tclistrd.getNoInput().getValueAsString();//未入力

      String[] rows = {
          user_name,        year,               month,          service_form,       work_day,
          work_hour,        overtime_day,       overtime_hour,  off_day,            off_hour,
          late_coming_day,  early_leaving_day,  absent_day,     paid_holiday,       compensatory_holiday,
          other_day,        noinput
          };
      rowcount = rowcount + 1;
