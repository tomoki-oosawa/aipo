dojo
    ._xdResourceLoaded({
      ...
      defineResource : function(dojo) {
        if (!dojo._hasResource["aipo.widget.DropdownActivityChecker"]) { // _hasResource
          // checks
          // added
          // by
          // build.
          ...
          /*
           * Aipo is a groupware program developed by Aimluck,Inc. Copyright (C)
           * 2004-2011 Aimluck,Inc. http://www.aipo.com
           *
          ...
          dojo
              .declare(
                  "aipo.widget.DropdownActivityChecker",
                  [ aimluck.widget.Dropdown ],
                  {
                    initValue : "",
                    displayCheck : "",
                    iconURL : "",
                    ...
                    postCreate : function() {
                      ...
                      this.dropDown = new aipo.widget.ActivityList({},
                          'activityLiteList');
                    },
                    _openDropDown : function() {
                      if (dojo.byId("tutorial_activity") != null) {
                        dojo.byId("tutorial_activity").style.display = "none"
                        dojo.query("a", dojo.byId("tutorial_activity"))[0]
                            .click()
                      }
                      this.inherited(arguments);
                      if (dojo.byId('activitycheckerContainer')) {
                        dojo.addClass(dojo.byId('activitycheckerContainer'),
                            'active');
                      }
                      ...
                      if (aipo.userAgent.isAndroid()) {
                        ...
                        // モーダルダイアログ
                        var dialogNodes = dojo.query("input,select,button,a",
                            this.domNode);

                        // ダイアログ表示時に機能しなくなるので、
                        // ヘッダとフッタ以下もpreventDefaultしない。
                        var navigationElems = dojo.byId("appsNavigation");
                        var navigationNodes = dojo.query(
                            "input,select,button,a", navigationElems);
                        var auiWidgetsElems = dojo.byId("auiWidgetsArea");
                        var auiWidgetsNodes = dojo.query(
                            "input,select,button,a", auiWidgetsElems);
                        ...
                        dojo.query("input,select,button,a").forEach(
                            function(node) {
                              if (!aipo.arrayContains(excludeNodes, node))
                                _this.eventList.push(dojo.connect(node,
                                    'click', f));
                            });
                        dojo.query("input,select,button").forEach(
                            function(node, index) {
                              if (!aipo.arrayContains(excludeNodes, node)
                                  && !node.disabled) {
                    ...
                    _closeDropDown : function() {
                      this.inherited(arguments);
                      if (dojo.byId('activitycheckerContainer')) {
                        dojo.removeClass(dojo.byId('activitycheckerContainer'),
                            'active');
                      }
                      ...
                      if (aipo.userAgent.isAndroid()) {
                        ...
                        dojo
                            .query(
                                "input.disabled-by-activity,select.disabled-by-activity,button.disabled-by-activity")
