...
public class ProjectTaskSelectData extends
  ...
  public void init(ALAction action, RunData rundata, Context context)
    ...
    try {
      selectedProjectId =
        Integer.valueOf(ALEipUtils.getTemp(rundata, context, LIST_FILTER_STR));
    } catch (Exception e) {
  ...
  protected ResultList<EipTProjectTask> selectList(RunData rundata,
    ...
    try {
      ...
      target_progress_rate_from =
        ProjectUtils
          .getParameter(rundata, context, "target_progress_rate_from");
      // 進捗率TO
      ...
      for (int i = 0; i < result.size(); i++) {
        ...
        // 進捗率
        task.setProgressRate(Integer.valueOf(row
          .get("progress_rate")
          .toString()));
        // 更新日
