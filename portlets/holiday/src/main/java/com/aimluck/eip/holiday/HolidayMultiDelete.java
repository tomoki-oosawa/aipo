...
/**
 * Holidayの複数削除を行うためのクラスです。 <BR>
 * 
 */
public class HolidayMultiDelete extends ALAbstractCheckList {

  /** logger */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(HolidayMultiDelete.class.getName());
  ...
  protected boolean action(RunData rundata, Context context,
    ...
    try {
      ...
      Expression exp1 =
        ExpressionFactory.inDbExp(EipMHoliday.HOLIDAY_ID_PK_COLUMN, values);

      List<EipMHoliday> holidayList =
        Database.query(EipMHoliday.class, exp1).fetchList();
