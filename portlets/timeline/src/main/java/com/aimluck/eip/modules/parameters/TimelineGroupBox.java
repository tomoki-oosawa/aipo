...
public class TimelineGroupBox extends ListBox {
  ...
  protected void init(RunData data) {
    ...
    try {
      ...
      /*
       * // 設備一覧の登録 groupKeys[count] = FACILITY_VALUE; groupValues[count] =
       * ALLocalizationUtils.getl10n("SCHEDULE_FACILITY_LIST"); count++;
       */
      // 部署の登録
      ALEipPost post = null;
      ...
      /*
       * // 設備グループの登録 ALEipGroup facilitygroup = null; Iterator<ALEipGroup>
       * iter3 = facilitygroupList.iterator(); while (iter3.hasNext()) {
       * facilitygroup = iter3.next(); groupKeys[count] = "f;" +
       * facilitygroup.getName().toString(); groupValues[count] =
       * facilitygroup.getAliasName().toString(); count++; }
       */
