...
dojo.declare("aipo.widget.MemberNormalSelectList", [dijit._Widget, dijit._Templated], {
    ...
    onBlur: function(select){
    	if(aipo.userAgent.isIphone()){
    	    var id = select.target.id;
    	    var list = [];
    	    for (var i = 0; i < select.target.length; i++) {
    	      if (select.target[i].selected) { 
    	    	  list.push(select.target[i].value); 
    	    	  }
    	    }
    	setTimeout(function(){
    		for (var i = 0; i < select.target.length; i++) {
    			select.target[i].selected = false;
    		}
    		
    		  for (var member in list) {
    			    for (var i = 0; i < select.target.length; i++) {
    			      if (select.target[i].value == list[member]) { 
    			    	  select.target[i].selected = true; 
    			    	  }
    			    }
    			  }
        }, 1000);
