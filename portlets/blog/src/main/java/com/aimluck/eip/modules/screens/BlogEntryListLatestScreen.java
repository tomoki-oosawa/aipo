...
/**
 * 新着ブログエントリーの一覧を処理するクラスです。 <br />
 * 
 */
public class BlogEntryListLatestScreen extends ALVelocityScreen {
  /** logger */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
      .getLogger(BlogEntryListLatestScreen.class.getName());
  ...
  protected void doOutput(RunData rundata, Context context) throws Exception {
    try {
      ...
      String layout_template = "portlets/html/ja/ajax-blog-entry-list-latest.vm";
      setTemplate(rundata, context, layout_template);

    } catch (Exception ex) {
      logger.error("[BlogEntryListLatestScreen] Exception.", ex);
