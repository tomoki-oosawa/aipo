...
public class CellAppScheduleSelectData {
  ...
  public void init(RunData rundata) {
    ...
    if (rundata.getParameters().containsKey(START_DATE)) {
      StringBuffer start_date = new StringBuffer(rundata.getParameters()
    ...
    if (rundata.getParameters().containsKey(END_DATE)) {
      StringBuffer end_date = new StringBuffer(rundata.getParameters()
          .getString(END_DATE));
  ...
  public boolean validate() {
    startDate.validate((ArrayList) msgList);
    endDate.validate((ArrayList) msgList);
    if (msgList.size() != 0) {
  ...
  private List selectList(RunData rundata) throws ALPageNotFoundException,
      ALDBErrorException {
    try {
      SelectQuery query = getSelectQuery(rundata);
      if (query == null)
        return null;
  ...
  private SelectQuery getSelectQuery(RunData rundata) {
    ...
    Expression exp11 = ExpressionFactory.greaterOrEqualExp(
        EipTScheduleMap.EIP_TSCHEDULE_PROPERTY + "."
            + EipTSchedule.END_DATE_PROPERTY, startDate.getValue());
    // 開始日時
  ...
  private Object getResultData(Object obj) throws ALPageNotFoundException,
      ALDBErrorException {
  ...
  private List getDayContainers(Date start, Date end) {
  ...
  private void addResultData(ScheduleResultData rd) {
    ...
    for (int i = 0; i < size; i++) {
      ScheduleDayContainer con = (ScheduleDayContainer) allContainer.get(i);
      ALDateTimeField field = con.getDate();

      if (rd.getPattern().equals("S")) {
        if (cals.before(cale)) {
          // 期間スケジュールの期間
          field_cal.set(Integer.parseInt(field.getYear()), Integer
              .parseInt(field.getMonth()), Integer.parseInt(field.getDay()), 0,
              0, 0);
  ...
  public void doFormatCsv() {
    ...
    for (int i = 0; i < size; i++) {
      ScheduleDayContainer con = (ScheduleDayContainer) allContainer.get(i);
      List alist = con.getScheduleList();
  ...
  public String outPutCsv() {
    ...
    int size = list.size();
    for (int i = 0; i < size; i++) {
      CellAppScheduleBean bean = (CellAppScheduleBean) list.get(i);
