...
public class ExtTimecardXlsExportScreen extends ALXlsScreen {
  ...
  public void init(RunData rundata, Context context)
    ...
    rootFolder = ExtTimecardUtils.getRootFolder(DatabaseOrmService.getInstance()
  ...
  private void setupTimecardSheet(RunData rundata, Context context,
    ...
    String[] headers = {
        "氏名",       "日付",       "曜日",       "勤務形態",         "出勤時間",
        "退勤時間",   "出勤日数",   "就業時間",   "残業日数",         "残業時間",
        "休出日数",   "休出時間",   "遅刻日数",   "早退日数",         "欠勤日数",
        "有休日数",   "代休日数",   "その他日数", "修正理由、備考",   "外出１",
        "復帰１",     "外出２",     "復帰２",     "外出３",           "復帰３",
        "外出４",     "復帰４",     "外出５",     "復帰５"
        };
    // 0：日本語，1：英数字
    short[] cell_enc_types = {
        HSSFCell.ENCODING_UTF_16, HSSFCell.ENCODING_UTF_16, HSSFCell.ENCODING_UTF_16, HSSFCell.ENCODING_UTF_16, HSSFCell.ENCODING_UTF_16,
        HSSFCell.ENCODING_UTF_16, HSSFCell.CELL_TYPE_NUMERIC, HSSFCell.CELL_TYPE_NUMERIC, HSSFCell.CELL_TYPE_NUMERIC, HSSFCell.CELL_TYPE_NUMERIC,
        HSSFCell.CELL_TYPE_NUMERIC, HSSFCell.CELL_TYPE_NUMERIC, HSSFCell.CELL_TYPE_NUMERIC, HSSFCell.CELL_TYPE_NUMERIC, HSSFCell.CELL_TYPE_NUMERIC,
        HSSFCell.CELL_TYPE_NUMERIC, HSSFCell.CELL_TYPE_NUMERIC, HSSFCell.CELL_TYPE_NUMERIC, HSSFCell.ENCODING_UTF_16, HSSFCell.ENCODING_UTF_16,
        HSSFCell.ENCODING_UTF_16, HSSFCell.ENCODING_UTF_16, HSSFCell.ENCODING_UTF_16, HSSFCell.ENCODING_UTF_16, HSSFCell.ENCODING_UTF_16,
        HSSFCell.ENCODING_UTF_16, HSSFCell.ENCODING_UTF_16, HSSFCell.ENCODING_UTF_16, HSSFCell.ENCODING_UTF_16
        };
    ...
    String user_name = ALEipUtils.getUserFullName(Integer.parseInt(listData.getTargetUserId()));//氏名
    ...
    List daykeys = listData.getDateListKeys();
    int daykeysize = daykeys.size();
    for (int i = 0; i < daykeysize; i++) {
      tclistrd = (ExtTimecardListResultData) daykeys.get(i);

      List viewlist = tclistrd.getViewList();
      ...
      String[] out_going = new String[EipTExtTimecard.OUTGOING_COMEBACK_PER_DAY];//外出
      String[] come_back = new String[EipTExtTimecard.OUTGOING_COMEBACK_PER_DAY];//復帰
      ...
      if(tclistrd.getRd() == null) {
        ...
        if(rd.getRefixFlag().getValue() == "1") {
          remark = (String)rd.getReason().getValue();
        }
        ...
        for(int j=0;j<EipTExtTimecard.OUTGOING_COMEBACK_PER_DAY;j++) {
          ...
          out_going[j] = (String)map.get("outgoing");
          come_back[j] = (String)map.get("comeback");
        }
      }

      String[] rows = {
          user_name,          date,               day,                service_form,   clock_in_time,
          clock_out_time,     work_day,          work_hour,    overtime_day,  overtime_hour,
          off_day,  off_hour,  late_coming_day,               early_leaving_day,          absent_day,
          paid_holiday,            compensatory_holiday,            other_day,          remark,         out_going[0],
          come_back[0],       out_going[1],       come_back[1],       out_going[2],   come_back[2],
...
/*
    rowcount = rowcount + 1;
    String NOTHING              = null;
    String work_day             = "=SUM(G2:G" + rowcount + ")";
    String work_hour            = "=SUM(H2:H" + rowcount + ")";
    String overtime_day         = "=SUM(I2:I" + rowcount + ")";
    String overtime_hour        = "=SUM(J2:J" + rowcount + ")";
    String off_day              = "=SUM(K2:K" + rowcount + ")";
    String off_hour             = "=SUM(L2:L" + rowcount + ")";
    String late_coming_day      = "=SUM(M2:M" + rowcount + ")";
    String early_leaving_day    = "=SUM(N2:N" + rowcount + ")";
    String absent_day           = "=SUM(O2:O" + rowcount + ")";
    String paid_holiday         = "=SUM(P2:P" + rowcount + ")";
    String compensatory_holiday = "=SUM(Q2:Q" + rowcount + ")";
    String other_day            = "=SUM(R2:R" + rowcount + ")";
    String[] rows = {
        NOTHING,      NOTHING,   NOTHING,          NOTHING,            NOTHING,
        NOTHING,      work_day,  work_hour,        overtime_day,       overtime_hour,
        off_day,      off_hour,  late_coming_day,  early_leaving_day,  absent_day,
        paid_holiday, compensatory_holiday,        other_day,
        };
