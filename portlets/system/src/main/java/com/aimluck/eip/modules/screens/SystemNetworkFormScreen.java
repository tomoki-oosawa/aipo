...
public class SystemNetworkFormScreen extends ALVelocityScreen {
  ...
  public void doSystem_form_network(RunData rundata, Context context)
    ...
    if ("global".equals(mode)) {
      ...
      setTemplate(rundata,context, "portlets/html/ja/ajax-system-form-network.vm");
    }
    if ("local".equals(mode)) {
      SystemNetworkInternalFormData formData = new SystemNetworkInternalFormData();
      formData.initField();
      formData.doViewForm(this, rundata, context);
      setTemplate(rundata,context, "portlets/html/ja/ajax-system-form-network-internal.vm");
    }
  }
}
