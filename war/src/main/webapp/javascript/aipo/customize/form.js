...
aipo.customize.showMenu = function(portlet_id) {
  ...
  if (menuNode.length == 0 || buttonNode.length == 0)
    return;// error
  ...
  if (menuNode.style('display') == 'none') {
    ...
    if (html.right - menuNode[0].clientWidth > rect.left) {
      ...
      menuNode.style("left", rect.right - menuNode[0].clientWidth + scroll.left
          + "px");
    }
    if (html.bottom - menuNode[0].clientHeight > rect.bottom) {
      menuNode.style("top", rect.bottom + scroll.top + "px");
    } else {
      menuNode.style("top", rect.top - menuNode[0].clientHeight + scroll.top
          + "px");
    }
    menuNode.style("opacity", "1");

    if (dojo.byId("timeline_" + portlet_id)
        && (dojo.query('div.timeline').length == 1)) {
...
aipo.customize.showMenuSchedule = function(portlet_id) {
  ...
  if (menuNode.style('display') == 'none') {
    ...
    aipo.customize.refixMenuScheduleListener = dojo.connect(window, 'onresize',
        function() {
...
aipo.customize.hideMenu = function(portlet_id) {
  ...
  if (subNavi) {
    var style = subNavi.currentStyle
        || document.defaultView.getComputedStyle(subNavi, null);
...
aipo.customize.addAutoHideMenuTrigger = function() {
  dojo.query('a.customizeMenuIcon,a.menubarOpenButton').forEach(
      function(element) {
