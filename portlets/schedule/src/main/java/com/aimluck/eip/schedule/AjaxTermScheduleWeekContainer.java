...
public class AjaxTermScheduleWeekContainer implements ALData {
  ...
  public void addTermResultData(int index, AjaxScheduleResultData rd) {
    ...
    AjaxTermScheduleDayContainer con = (AjaxTermScheduleDayContainer) dayList
        .get(index);
    con.setHasTerm(true);
    con.setTermResultData(rd);
    for (int i = 1; i < span; i++) {
      con = (AjaxTermScheduleDayContainer) dayList.get(index + i);
      con.setHasTerm(true);
    }
  }
  ...
  public boolean canAddTerm(AjaxScheduleResultData rd) {
    ...
    for (int i = 0; i < size; i++) {
      con = (AjaxTermScheduleDayContainer) dayList.get(i);
      Date conDate = con.getDate().getValue();
      boolean isEmptyStartDate = ScheduleUtils.equalsToDate(ScheduleUtils.getEmptyDate(),
          startDate, false);
      boolean isEmptyEndDate = ScheduleUtils.equalsToDate(ScheduleUtils.getEmptyDate(),
          endDate, false);
