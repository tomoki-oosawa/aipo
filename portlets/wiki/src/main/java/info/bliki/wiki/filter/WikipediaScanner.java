...
public class WikipediaScanner {
  ...
  public WPCodeBlock codeBlock() {
    WPCodeBlock code = null;
    try {
      ...
      if (fSource[fScannerPosition++] != 'c'
        || fSource[fScannerPosition++] != 'o'
        || fSource[fScannerPosition++] != 'd'
              ...
              case '|': // "\n |"
                ...
                switch (ch) {
                  case '-': // new row - "\n|-"
                    ...
                    row.setParams(fStringSource.substring(
                      startPos,
                      fScannerPosition));
                    break;
                  case '+': // new row - "\n|+"
    ...
    try {
      ...
      while (true) {
        if (ch == WPList.DL_DD_CHAR
          && lastCh == WPList.DL_DT_CHAR
          && sequence != null) {
          startPosition = fScannerPosition;
          if (listElement != null) {
            listElement.createTagStack(
              fSource,
              fWikiModel,
              fScannerPosition - 1);
        ...
        if (ch == '\n' || fScannerPosition == 0) {
          ...
          switch (ch) {
            ...
            case WPList.UL_CHAR:
              count = 1;
              lastCh = ch;
              while (fSource[fScannerPosition] == WPList.UL_CHAR
                || fSource[fScannerPosition] == WPList.OL_CHAR
                || fSource[fScannerPosition] == WPList.DL_DD_CHAR
                || fSource[fScannerPosition] == WPList.DL_DT_CHAR) {
                count++;
  ...
  public int indexEndOfNowiki() {
    ...
    while (fScannerPosition < fSource.length - 8) {
      ...
      if (ch == '<'
        && fSource[fScannerPosition] == '/'
        && fSource[fScannerPosition + 1] == 'n'
        && fSource[fScannerPosition + 2] == 'o'
        && fSource[fScannerPosition + 3] == 'w'
        && fSource[fScannerPosition + 4] == 'i'
        && fSource[fScannerPosition + 5] == 'k'
        && fSource[fScannerPosition + 6] == 'i'
        && fSource[fScannerPosition + 7] == '>') {
    ...
    try {
      while (fScannerPosition < fSource.length) {
        ...
        if (ch == '<'
          ...
          && fSource[fScannerPosition] == 'n'
          && fSource[fScannerPosition + 1] == 'o'
          && fSource[fScannerPosition + 2] == 'w'
          && fSource[fScannerPosition + 3] == 'i'
          && fSource[fScannerPosition + 4] == 'k'
          && fSource[fScannerPosition + 5] == 'i'
          && fSource[fScannerPosition + 6] == '>') {
          // start of <nowiki>
          fScannerPosition += 7;
   ...
   *         <code>null</code>
   */
  public static boolean startsWith(String str, int toffset, String prefix,
      boolean ignoreCase) {
   ...
   */
  public StringBuilder replaceTemplateParameters(String template,
      ...
      while (true) {
        ...
        if (ch == '{'
          ...
          if (temp[0] >= 0) {
            ...
            List<String> list =
              splitByPipe(fSource, parameterStart, fScannerPosition - 3, null);
            if (list.size() > 0) {
              ...
              if (value != null) {
                if (value.length() <= Configuration.TEMPLATE_VALUE_LIMIT) {
                  ...
                  if (bufferStart < fScannerPosition) {
                    buffer.append(fSource, bufferStart, parameterStart
                      - bufferStart
                      - 3);
                  }
                  ...
                  if (isDefaultValue) {
                    recursiveResult =
                      scanner.replaceTemplateParameters(
                        value,
                        templateParameters);
                  } else {
                    recursiveResult =
                      scanner.replaceTemplateParameters(value, null);
                  }
        ...
        if (buffer != null
          && buffer.length() > Configuration.TEMPLATE_BUFFER_LIMIT) {
   ...
   * @return
   */
  public static List<String> splitByPipe(String sourceString,
   ...
   * @return
   */
  public static List<String> splitByPipe(char[] srcArray, int currOffset,
      int endOffset, List<String> resultList) {
    if (resultList == null) {
    ...
    try {
      while (currOffset < endOffset) {
        ...
        if (ch == '[' && srcArray[currOffset] == '[') {
          ...
          resultList.add(new String(srcArray, lastOffset, currOffset
            - lastOffset
            - 1));
      ...
      if (currOffset > lastOffset) {
        resultList
          .add(new String(srcArray, lastOffset, currOffset - lastOffset));
   ...
   *         found
   */
  public static int findNestedEnd(final char[] sourceArray, final char startCh,
      final char endChar, int startPosition) {
    char ch;
    int level = 1;
   ...
   *         found
   */
  public static int findNestedEndSingle(final char[] sourceArray,
  ...
  public static int findNestedTemplateEnd(final char[] sourceArray,
      int startPosition) {
    char ch;
  ...
  public static int[] findNestedParamEnd(final char[] sourceArray,
    ...
    try {
      while (parameterPosition < sourceArrayLength) {
        ...
        if (ch == '{') {
          if ((sourceArrayLength > parameterPosition)
            && sourceArray[parameterPosition] == '{') {
            parameterPosition++;
            if ((sourceArrayLength > parameterPosition)
              && sourceArray[parameterPosition] == '{'
   ...
   * &lt;/H1&gt;
   * </code> By default, SGML requires that all attribute values be delimited
   * using either double quotation marks (ASCII decimal 34) or single quotation
   * marks (ASCII decimal 39). Single quote marks can be included within the
   * attribute value when the value is delimited by double quote marks, and vice
   * versa. Authors may also use numeric character references to represent
   * double quotes (&amp;#34;) and single quotes (&amp;#39;). For doublequotes
   ...
   */
  protected WikiTagNode makeTag(int start, int end,
   ...
   * @return
   */
  protected final int readUntilIgnoreCase(int start, String startString,
      String endString) {
    int index =
      Util.indexOfIgnoreCase(fStringSource, startString, endString, start);
    if (index != (-1)) {
