...
public class ScheduleExportListContainer implements ALData {
  ...
  private ALDateTimeField viewEndDate;

  private List<ScheduleExportResultData> scheduleList;
  ...
  public void setViewEndDate(Calendar cal) {
    viewEndDate.setValue(cal.getTime());
  }

  @Override
  public void initField() {
    viewStartDate = new ALDateTimeField("yyyy-MM-dd-HH-mm");
    viewEndDate = new ALDateTimeField("yyyy-MM-dd-HH-mm");
    // スケジュールリスト
  ...
  public List<ScheduleExportResultData> getScheduleList() {
    if (!isSort) {
      // ソート
    ...
    List<ScheduleExportResultData> results =
      new ArrayList<ScheduleExportResultData>();
    for (ScheduleExportResultData rd : scheduleList) {
      if (!rd.isDummy()) {
        results.add(rd);
      }
    }
