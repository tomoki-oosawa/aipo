...
public class MultiColumnControllerAction extends VelocityControllerAction {
  ...
  protected void buildNormalContext(PortletController controller,
    ...
    try {
      ...
      String columnClasses = controller.getConfig().getInitParameter(
          "col_classes");
      ...
      while (en.hasMoreElements()) {
        ...
        PortletSet.Constraints constraints = p.getPortletConfig()
            .getConstraints();

        if ((constraints != null) && (constraints.getColumn() != null)
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("Controller calculated setSize " + set.size() + " row "
            + row + " colNum: " + colNum + " rowNum: " + rowNum);
      }
  ...
  public void doSave(RunData data, Context context) {
    ...
    try {
      ...
      // continue
      // being displayed)
      String referencesRemoved = (String) customizationState
          .getAttribute(REFERENCES_REMOVED);
  ...
  public void doDelete(RunData data, Context context) {
    ...
    // アクセス権限のチェック
    ALEipUtils.CheckAclPermissionForCustomize(data, context,
    ...
    if ((col > -1) && (row > -1)) {
      try {
        ...
        Portlets portlets = jdata.getCustomizedProfile().getDocument()
        ...
        logger.error(
            "MultiColumnControllerAction: Probably got wrong coordinates", e);
      }
    }
  }
