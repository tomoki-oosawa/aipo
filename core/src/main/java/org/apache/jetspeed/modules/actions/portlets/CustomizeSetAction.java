...
public class CustomizeSetAction extends VelocityPortletAction {
  ...
  protected void buildNormalContext(VelocityPortlet portlet, Context context,
    ...
    if (mode == null) {
      ...
      if ((mode == null) || (mode.equalsIgnoreCase("addset"))
    ...
    if (template != null) {
      ...
      template = TemplateLocator.locatePortletTemplate(rundata, buffer
          .toString());
    ...
    String customizedPaneName = (String) customizationState
        .getAttribute("customize-paneName");
    ...
    if (mediaType.equalsIgnoreCase("wml")) {
      ...
      if (set.getPortletConfig().getSkin() != null) {
        context.put("currentSkin", set.getPortletConfig().getPortletSkin()
            .getName());
      }
    ...
    {
      ALEipUtils.CheckAclPermissionForCustomize(rundata, context, 
  ...
  public int getSize(VelocityPortlet portlet) {
    int size = 30;
    try {
      size = Integer.parseInt(portlet.getPortletConfig().getInitParameter(
          "size"));
    } catch (Exception e) {
  ...
  public void doAddset(RunData rundata, Context context) {
    ...
    if (portlets != null) {
      ...
      SecurityReference defaultRef = PortalToolkit
          .getDefaultSecurityRef(((JetspeedRunData) rundata)
  ...
  protected void maintainUserSelections(RunData rundata) throws Exception {
    ...
    Map userSelections = getUserSelections(rundata);
    List portlets = (List) PortletSessionState.getAttribute(rundata,
        PORTLET_LIST, null);
    if (portlets != null) {
      int end = Math.min(start + size, portlets.size());
      int pnamesIndex = 0;
      ...
      for (int portletIndex = start; portletIndex < end; portletIndex++) {
        ...
        if (pnames != null && pnamesIndex < pnames.length
  ...
  public void doAdd(RunData rundata, Context context) throws Exception {
    ...
    if ((pnames != null) && (set != null)) {
      Portlets portlets = ((JetspeedRunData) rundata).getCustomizedProfile()
          .getDocument().getPortletsById(set.getID());
    ...
    rundata.setRedirectURI(jsLink.getTemplate().addPathInfo("js_peid",
        set.getID()).addPathInfo("mode", "layout")
        .addQueryData("mtype", "html").addQueryData("action",
            "controls.Customize").toString());
     ...
     * --------------------------------------------------------------------------
     * last modified: 10/31/01 Andreas Kempf, Siemens ICM S CP PE, Munich
     * mailto: A.Kempf@web.de if (isWML) {
    ...
    if ((refNames != null) && (set != null)) {
      ...
      for (int i = 0; i < refNames.length; i++) {
        ...
        if (sref != null) {
          ...
          String tmpl = CustomLocalization.getString(
              "CUSTOMIZER_ADD_REF_ERROR", rundata);
    ...
    try {
      ...
      if (controller != null) {
        ...
        if (pc != null) {
          ...
          Portlets portlets = profile.getDocument()
    ...
    try {
      ...
      if ((skin != null) && (skin.trim().length() > 0)) {
        ...
        if (s != null) {
          ...
          logger.warn("Unable to update skin for portlet set " + set.getID()
        ...
        if (custPortletSetID != null && rootPortletSetID != null
          ...
          String defaultSkinName = JetspeedResources
              .getString("services.PortalToolkit.default.skin");
          ...
          if (defaultSkin != null) {
            ...
            logger.warn("Unable to set default skin for root portlet set "
                + set.getID() + " because skin " + skin + " does not exist.");
            return;
          }
        } else {
  ...
  public static List buildPortletList(RunData data, PortletSet set,
      String mediaType, List allPortlets) {
    ...
    while (i.hasNext()) {
      ...
      if (JetspeedSecurity.checkPermission((JetspeedUser) data.getUser(),
          new PortalResource(entry), JetspeedSecurity.PERMISSION_VIEW)
          && ((!entry.isHidden())
    ...
    Collections.sort(list, new Comparator() {
      public int compare(Object o1, Object o2) {
        String t1 = (((PortletEntry) o1).getTitle() != null) ? ((PortletEntry) o1)
            .getTitle().toLowerCase()
            : ((PortletEntry) o1).getName().toLowerCase();
        String t2 = (((PortletEntry) o2).getTitle() != null) ? ((PortletEntry) o2)
            .getTitle().toLowerCase()
  ...
  public static Map getUserSelections(RunData data) {
    Map userSelections = (Map) PortletSessionState.getAttribute(data,
  ...
  public static List buildInfoList(RunData data, String regName,
    ...
    String mime = ((JetspeedRunData) data).getCapability().getPreferredType()
        .toString();
    ...
    while (i.hasNext()) {
      ...
      // MODIFIED: Selection now takes care of the specified mediatype!
      if (JetspeedSecurity.checkPermission((JetspeedUser) data.getUser(),
          new PortalResource(entry), JetspeedSecurity.PERMISSION_CUSTOMIZE)
    ...
    Collections.sort(list, new Comparator() {
      public int compare(Object o1, Object o2) {
        String t1 = (((RegistryEntry) o1).getTitle() != null) ? ((RegistryEntry) o1)
            .getTitle()
            : ((RegistryEntry) o1).getName();
        String t2 = (((RegistryEntry) o2).getTitle() != null) ? ((RegistryEntry) o2)
            .getTitle()
   ...
   */
  public static List buildCategoryList(RunData data, String mediaType,
      List portlets) {
  ...
  public void doFilter(RunData rundata, Context context) throws Exception {
    ...
    for (int i = 0; i < filterFields.length; i++) {
      ...
      if (filterField != null) {
        String filterValue = rundata.getParameters().getString(
  ...
  private void setPageLayout(RunData rundata, Context context) {
    ...
    Portlets set = jdata.getCustomizedProfile().getDocument().getPortletsById(
        customizedSet.getID());

    if (logger.isDebugEnabled()) {
      logger.debug("MultiCol: columns " + Arrays.toString(columns) + " set "
          + set);
    }
    if ((columns != null) && (columns.length == colNum)) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("MultiCol: eCount " + eCount + " setCount"
            + set.getEntryCount() + set.getPortletsCount());
      }
    ...
    for (int col = 0; col < columns.length; col++) {
      for (int row = 0; row < columns[col].size(); row++) {
        IdentityElement identityElement = (IdentityElement) columns[col]
            .get(row);
        ...
        if (identityElement instanceof Entry) {
          ...
          PortletEntry pentry = (PortletEntry) Registry.getEntry(
              Registry.PORTLET, entry.getParent());
  ...
  protected static List[] buildColumns(Portlets set, int colNum) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("Controller customize colNum: " + colNum + " rowNum: "
          + rowNum);
    }
