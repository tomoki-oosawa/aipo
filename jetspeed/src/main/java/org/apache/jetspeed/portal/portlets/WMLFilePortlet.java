...
/**
 * This portlet serves well-formed WML file content and strips them of
 * superfluous tags liks <wml>
 *
 * <strong>The strip capability has been temporarily disabled due to class
 * conflicts in Tomcat 3.2, so this portlet can only currently serve fragments
 * of WML documents, without the <wml> tag</strong>
 *
...
public class WMLFilePortlet extends FileWatchPortlet {
   ...
   *
   */
  private static final long serialVersionUID = -5600303707781491326L;

  /**
   * Static initialization of the logger for this class
   */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(WMLFilePortlet.class.getName());
  ...
  public void init() throws PortletException {
    ...
    try {
      ...
      // with Tomcat 3.2
      /*
       * String url = JetspeedDiskCache.getInstance().getEntry( config.getURL()
       * ).getURL();
       * 
       * WMLFilter myFilter = new WMLFilter(new PrintWriter(bos));
       * myFilter.filter(url); content = new JetspeedClearElement(
       * bos.toString() );
       */
      content =
        new JetspeedClearElement(JetspeedDiskCache.getInstance().getEntry(
          config.getURL()).getData());
    } catch (Exception e) {
  ...
  class WMLFilter extends DefaultHandler {
